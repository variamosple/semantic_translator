...
"""
        ...
        with PrologMQI() as mqi:
            with PrologThread(mqi) as prolog_thread:
                ...
                print('file loaded')
                result = prolog_thread.query("program([!!!]), once(label([!!!])).".replace('!!!',','.join(occs)))
                print(result)
                return result
...
def prolog_update_model(model, rules, result):
    for e in model["elements"]:
        if e["type"] in rules["elementTypes"] and e["properties"][1]["value"] not in ['Selected', 'Unselected']:
            e["properties"][1]["value"] = "SelectedForced" if result[0]["UUID_" +  str(e["id"]).replace("-","_")] == 1 else  "UnselectedForced"
